(function ($) {
  ...
  function showStep(stepNo) {
    ...
    $(".cap-step-indicator")
      ...
      .each(function () {
        ...
        if (s < stepNo) $(this).addClass("completed");
        if (s === stepNo) $(this).addClass("active");
      });
  ...
  function validateStep(stepNo, renderInlineErrors = false) {
    ...
    if (renderInlineErrors) clearFieldErrors($step);
    ...
    $step.find("[required]").each(function () {
      ...
      if ($field.is(":checkbox")) {
        if (!$field.is(":checked")) {
          valid = false;
          if (renderInlineErrors) showFieldError($field, "This checkbox is required");
        }
        return;
      }
      ...
      if (!$field.val()) {
        valid = false;
        if (renderInlineErrors) showFieldError($field, `${getFieldLabel($field)} is required`);
      }
    });
  ...
  $(document).on("submit", ".cap-reg-form", function (e) {
    ...
    if (!validateStep(1, true)) return showStep(1);
    if (!validateStep(2, true)) return showStep(2);
    if (!validateStep(3, true)) return showStep(3);
  ...
  /* ---------------------------
	   RAZORPAY FLOW (UNCHANGED CORE)
	----------------------------*/
  $(document).on("click", "#cap-pay-now", function () {
    if (!checkoutSession) return alert("Submit form first");
